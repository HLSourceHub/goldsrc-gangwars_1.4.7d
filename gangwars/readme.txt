Latest version: 1.47.1b (second release of 1.47)
...
December 08, 2017 - December 28, 2019
...
Version 1.47.1d:
+Change gw_freebies to only give primary/secondary weapons when a player doesn't already have a primary/secondary weapon
+Fixed gw_roundtime settings
+Adding Automatic Team Balancing, but has not yet been implemented properly ("mp_teambalance")
+Added support for "Team Balance" option to team selection menu
+Add some bot support for a custom version of PodBot designed for use with Gangwars instead of Counter-Strike
+Added minimal bot support for a custom version of HPB Bot designed for use with Gangwars (removed, use custom PodBot)
+Change how team selection works when a player is alive - such as joining "spectator"
+Added "Ghettonade" boss weapon +bug fixes (only BOSS was supposed to be able to purchase Ghettonades - dropping disabled)
+Change how weapon switching and selection works (can still switch to weapons with empty ammo, to drop weapons)
+Add "freezetime" to control players during first couple seconds of the round
+Add log messages for events in the game (log events)
+Add some snow texture footstep sound support
+Known bug: Metamod will no longer load on Linux listenserver
+Known bug: FMOD 3.75 may not work properly with some Linux sound card settings or drivers

Version 1.47.1c:
+Clean parts of the code and rebuild binaries after a hard drive crash.
+Code is now compatible with Visual Studio 2017 and possibly GCC 5
+Changes made to team selection menu usage (player must select model before player can spawn - prevents team change bug where player could use a model from the wrong team)
+Update decals.wad to change hostage rescue zone to say "Gangwars" instead of old decal
